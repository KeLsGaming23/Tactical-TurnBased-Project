...
namespace kelsgaming.site
{
    public class TurnSystem : MonoBehaviour
    {
        ...
        public void StartNextTurn()
        {
            ...
            // Find units that have not yet acted this round
            List<Unit> availableUnits = allUnitsList.FindAll(u => !u.HasActedThisRound());

            // If all units have acted, start a new round!
            if (availableUnits.Count == 0)
            {
                roundNumber++;
                foreach (Unit unit in allUnitsList)
                {
                    unit.SetHasActedThisRound(false);
                }
            ...
            // "if the speed value of the current unit double its value from the next unit it has the ability to have double the move"
